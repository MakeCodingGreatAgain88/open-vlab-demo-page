...
const DataTable: React.FC<DataTableProps> = React.memo(({ data, loading }) => {
  ...
  const renderColumnTitle = React.useCallback((key: string, title: string) => {
    return (
      <>
        <span>{title}</span>
        <span 
    ...
    },
    {
      ...
      render: (_: any, record: MarketDataItem) => (
        <div className="chart-preview-wrapper">
          <MiniChart data={record.chartData} height={60} />
        </div>
      ),
    },
  ], [sortedInfo, renderColumnTitle]);
